...
`--icon-source /path/to/web-icon.png` дополнительно
сравнивает иконку побайтно с эталоном веб-ресурса 128×128. Список внешних URL в выводе —
...
Веб-иконка `assets/kilogram-app-icon.png` намеренно уменьшена до 128×128
в коммите `77d7b90` («Уменьшена иконка приложения»). Контракт валидатора —
PNG 128×128, 8 бит на канал, RGBA с alpha. В HTML и CSS она отображается
в размере 40×40 CSS-пикселей без изменения пропорций (`object-fit: contain`);
для плотности 2× нужны 80×80 пикселей, для 3× — 120×120, что укладывается в ресурс.
Все страницы EN/RU, общая 404, favicon и apple-touch-icon используют
[один ассет](https://rinatabidullin.github.io/kilogram-site/assets/kilogram-app-icon.png).
Размер и качество значка, который ОС создаёт из apple-touch-icon, отдельно не проверены.
Оригинал 1024×1024 сохранён в истории Git до `77d7b90`; увеличивать веб-иконку
до этого размера ради прохождения проверки не нужно.
